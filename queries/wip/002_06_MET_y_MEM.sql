...
-- CAPA DE MEMORIA
-- ======================
-- Es una copia estandarizada y persistente de las TMP_, sin cambios de semántica.
-- Sirve para: mantener el histórico de lo que viene del OLTP, desacoplar limpieza/derivados del dato original, asegurar trazabilidad
-- Aquí NO se derivan campos, NO se alteran tipos, NO se pierden columnas.

-- regions
DROP TABLE IF EXISTS DWM_regions;
CREATE TABLE DWM_regions AS
SELECT *
FROM TMP_regions;
-- territories
DROP TABLE IF EXISTS DWM_territories;
CREATE TABLE DWM_territories AS
SELECT *
FROM TMP_territories;
-- categories
DROP TABLE IF EXISTS DWM_categories;
CREATE TABLE DWM_categories AS
SELECT *
FROM TMP_categories;
-- suppliers
DROP TABLE IF EXISTS DWM_suppliers;
CREATE TABLE DWM_suppliers AS
SELECT *
FROM TMP_suppliers;
-- shippers
DROP TABLE IF EXISTS DWM_shippers;
CREATE TABLE DWM_shippers AS
SELECT *
FROM TMP_shippers;
-- employees
DROP TABLE IF EXISTS DWM_employees;
CREATE TABLE DWM_employees AS
SELECT *
FROM TMP_employees;
-- employee_territory
DROP TABLE IF EXISTS DWM_employee_territories;
CREATE TABLE DWM_employee_territories AS
SELECT *
FROM TMP_employee_territories;
-- customers
DROP TABLE IF EXISTS DWM_customers;
CREATE TABLE DWM_customers AS
SELECT *
FROM TMP_customers;
-- products
DROP TABLE IF EXISTS DWM_products;
CREATE TABLE DWM_products AS
SELECT *
FROM TMP_products;
-- orders
DROP TABLE IF EXISTS DWM_orders;
CREATE TABLE DWM_orders AS
SELECT *
FROM TMP_orders;
-- order_details
DROP TABLE IF EXISTS DWM_order_details;
CREATE TABLE DWM_order_details AS
SELECT *
